...
const Projects = ({indexValue}: ProjectsProps)  => {
    return (
        <div className={`group flex flex-col mx-auto relative mt-6 overflow-hidden w-full h-[55vh] cursor-pointer rounded-lg border border-gray-600 bg-cover ${arrayProjects[indexValue].pos} bg-no-repeat`}
        style={{backgroundImage: `url(/${arrayProjects[indexValue].image})`}}>
            <span className="bg-gradient-to-t absolute from-[#0e0e0e] from-50% to-transparent w-full h-full translate-y-[65%] group-hover:translate-y-[55%] duration-500"></span>
            ...
            <ul className="absolute -bottom-12 left-6 group-hover:bottom-3 z-10 duration-500">
                <h3 className={`${geistSans.className} text-2xl font-bold `}> { arrayProjects[indexValue].name } </h3>
                <div>
                    <ul className="flex gap-4 my-3 w-4/6">
                        { arrayProjects[indexValue].tecnologies.map((item, index) => {
                            return (
                                <li key={index} className={`${geistSans.className} flex gap-2 text-lg items-center px-3 py-1 rounded-lg border border-gray-600`}>
                                    {item.icon}
                                    <p> {item.name} </p>
                                </li>
                            )
                    ...
                    <div className="w-3/4">
                        <p className={`${geistSans.className} text-neutral-300 text-lg font-semibold`}> { arrayProjects[indexValue].description } </p>
                    </div>
